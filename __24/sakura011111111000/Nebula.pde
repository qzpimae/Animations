...
class Nebula {
  ...
  void render (float radius, float nebHeight) {
    ...
    for (float i = 0; i < layers; i++) {
      ...
      for (float j = 0; j < starsInLayer; j++) {
        ...
        float xOff = map(cos(map(j, 0, starsInLayer, -PI, PI)), -1, 1, 1, 13);
        ...
        float hueN = (float) noise.noise3_Classic(xOff +3324+ nebulaTime/23, yOff+3724+ nebulaTime/17, zOff +3924); //+ nebulaTime +3424 ,  + 3424
  ...
  void render1 (float radius, float nebHeight) {
    ...
    for (float i = 0; i < layers; i++) {
      ...
      for (float j = 0; j < starsInLayer; j++) {
        ...
        if (z < 0){
          ...
          float noisenoise = (float) noise.noise3_Classic(xOff +132- nebulaTime/35, yOff+421 + nebulaTime/7, zOff + nebulaTime/10 +312) / 5678 + 1; //+ nebulaTime +3424 ,  + 3424
          
          float noiseX = xOff * noisenoise + 3324 + nebulaTime/23;
          float noiseY = yOff * noisenoise + 3724 + nebulaTime/17;
          float noiseZ = zOff +3924;
  
          float hueN = (float) noise.noise3_Classic(noiseX, noiseY, noiseZ); //+ nebulaTime +3424 ,  + 3424
          ...
          if (hueN < 0) {
            ...
            hue = abs(map(hueN, -1, 1, 0, 5000) + map(abs(y), 0, nebHeight, 5000, 1000)) % 10000;
            sat = map(abs(y), 0, nebHeight, 40, 85);
            light = map(abs(y), 0, nebHeight, 100, 0) * (hueN);
          } else {
            //org
