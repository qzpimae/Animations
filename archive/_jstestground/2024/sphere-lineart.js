...
let canvas = document.createElement('canvas');
    ...
    pointSize = 1, //size of the dots
    autoAddPoints = false, //user can toggle auto adding points
    lightToggle = true, //if dots are black or white (color mode unaffected)
    ...
    document.addEventListener('keydown', (evn) => {
        ...
        //USED: z,x,r,f,t,g,y,h,c,v,
            ...
            pointSize -= .05;  
        } else if (evn.code == 'KeyV') {
            pointSize += .05;  
        } else if (evn.code == 'KeyM') {
   ...
   //ANIMATION CYCLE
    ...
    function renderPoint(origin) {
        ...
        context.fillStyle = colorBool ? `hsl(${frames}, 100%, 50%)`: `hsla(0, 100%, ${lightToggle ? 100 : 0}%, ${(frames/200 * mapNumber(pointsNum, 1, 100, .5, .01))})`;//'white';

        let size = pointSize < .01 ? .01 : pointSize;// 2-pointsNum/10 > .7 ? 2-pointsNum/10 : .7;
