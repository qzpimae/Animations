...
function createImg(s) { 
    ...
    let mNoise = Noise(s/333+seed,s/333+seed)*3*(1+seed/10),
        light = (frames % 2 == 0 ? -100 : 0 )+ mapNumber(time, 0, timeMax, 0, 100);
	light *= Math.sin(time/100) + 1;
    context.lineWidth = Math.random()*3;
    context.strokeStyle = `hsla(0, 0%, ${light}%, .1)`
