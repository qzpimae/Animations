...
let canvas = document.createElement('canvas');
    ...
    document.addEventListener('keydown', (evn) => {
        ...
        switch (evn.code) {
            case 'ArrowLeft':
                moveHor = moveHor == -1 ? 0 : -1;
                break;
                case 'ArrowRight':
                    moveHor = moveHor == 1 ? 0 : 1;
                break;
                case 'ArrowUp':
                    moveVer = moveVer  == 1 ? 0 : 1;
                break;
                case 'ArrowDown':
                    moveVer  == -1 ? 0 : -1;
                break;
    ...
    context.fillStyle = 'aquamarine';//`hsl(${}, 100%, 70%)`
   ...
   create_star_field()
    ...
    function createGrid() {
        ...
        for (let i = 2; i < originHeight; i*=(1+ height/8000) + frames/3300) {
            
            context.beginPath()
            context.moveTo(originWidth,originHeight + i)
            context.lineTo(width,originHeight + i)
            context.stroke()
        ...
        for (let i = (10+(frames/100))/2; i < width/2; i+=10+(frames/100)) {
            ...
            context.beginPath()
            context.moveTo(width/2 - i,originHeight+2)
    ...
    function create_star_field() {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            let x = (width * Math.random())-width/2,
                y = (originHeight * Math.random())-height/4,
