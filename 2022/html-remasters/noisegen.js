...
function userInputEvent(input) {
    ...
    switch (input.code) {
        ...
        case 'KeyT':
            colorControl = colorControl > 0 ? colorControl-1 : 360;
            // console.log(colorControl);         
        break;
        case 'KeyY':
            colorControl = colorControl < 360 ? colorControl+1 : 0;
            // console.log(colorControl);         
        break;
        case 'KeyU':
            colorRange = colorRange > 0 ? colorRange-10 : 0;
            // console.log(colorControl);         
        break;
        case 'KeyI':
            colorRange = colorRange < 360*10 ? colorRange+10 : 360*10;
            // console.log(colorControl);         
        break;
        case 'KeyS':
    ...
    for (let x = 0; x < viewWidth; x+=pixSize) {
        ...
        for (let y = 0; y < viewHeight; y+=pixSize) {
          ...
          const offX = (mapNumber(nRes, 3, timeMax, 0, width*4)/nRes)+transX,
                ...
                noiseBase = Noise(noiseX, noiseY),
                light = (Math.abs((noiseBase*divid )) % divid)+nRes/3.3+y/12,
                color = mapNumber(noiseBase, -1, 1, colorControl, colorControl+colorRange);//(Math.abs((noiseBase*nRes/5 )*nRes/3));
