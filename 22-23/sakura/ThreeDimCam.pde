...
class ThreeDimCam {
    ...
    if (camOpt == 0) {
       //AUTO ROTATE CAM
         ...
         curX = 0;//sin((float)frames/111*cameraSpeed) * radius;
          ...
          float titleRangeMax = 78;
          float titleRangeMin = 30.5;
          
          //Zoom head on cam     
          float zoomSpeed = map(radius, 30, 1400, .1, .9);
         ...
         if (radius > 1000) radius -= .217 * renderSpeed;
  ...
  void renderCamGumball() {
    ...
    // fill(10000);
    // box(testRadius/25, testRadius/25, testRadius/25);
  }
